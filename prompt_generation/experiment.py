...
if __name__ == "__main__":
    ...
    best_generated_df = pd.DataFrame(columns = ["pairs", "loss", "sen_sim"])
    ...
    for i in tqdm.tqdm(range(args.num_iterations)):
      ...
      helpers.normalize_column(temp_df, "loss")

      temp_df["score"] = temp_df["normalized_loss"] + temp_df["sen_sim"]
      temp_df.sort_values(by = ["score"], inplace = True)
